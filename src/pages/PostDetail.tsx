...
export default function PostDetail({ posts, onVote, onReaction, onComment }: PostDetailProps) {
  const { id } = useParams()
  ...
  if (!post) return (
    <div style={{ textAlign: 'center', padding: '80px 20px' }}>
      ...
      <Link to="/feed" className="btn btn-primary" style={{ marginTop: 20, display: 'inline-flex' }}>
        Back to Feed
      </Link>
    </div>
  )
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: '32px 0 60px' }}>
      <div className="container" style={{ maxWidth: 720 }}>
        ...
        <PostCard post={post} onVote={onVote} onReaction={onReaction} onComment={onComment} showFull={true} />
      </div>
    </div>
  )
}
