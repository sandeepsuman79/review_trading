...
const TYPES = ['All', 'banner', 'trailer', 'book', 'leader'] as const
const FREQS = ['All', 'daily', 'weekly', 'monthly'] as const

type FeedPageProps = {
  posts: PostItem[]
  onVote: (postId: number, option: string) => void
  onReaction: (postId: number, type: string) => void
  onComment: (postId: number, comment: import('../data/posts').CommentItem) => void
}
...
export default function FeedPage({ posts, onVote, onReaction, onComment }: FeedPageProps) {
  const [typeFilter, setTypeFilter] = useState<typeof TYPES[number]>('All')
  const [freqFilter, setFreqFilter] = useState<typeof FREQS[number]>('All')
      ...
      <div className="container">
        {filtered.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 20 }}>
            {filtered.map((post) => (
              <PostCard key={post.id} post={post} onVote={onVote} onReaction={onReaction} onComment={onComment} />
            ))}
          </div>
        )}
      </div>
